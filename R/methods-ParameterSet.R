...
setMethod("getQuantilesMixture", signature(object = "ParameterSet"), function(object){
    df <- ITHIM:::getQuantilesMixtureFunction(p.AT = object@pAT, mean.AT = object@meanLogTA, sd.AT = object@sdLogTA, mean.NT = object@meanLogLA, sd.NT = object@sdLogLA, prob = object@quantiles)
    return(df$quantiles)
})
